...
ISR (TIMER1_COMPA_vect)
{
	...
	if(!(PIND & 0b00010000)) cnt = 0; //Если вдруг обнаружим низкое состояние ножки кнопки (кнопка нажата), то обнулим счётчик
...
void ledprint(unsigned int number, unsigned char Rl[4])
{
	for (uint8_t i = 0; i < sizeof(R) / sizeof(R[0]); i++)
	{
		Rl[P(i)] = '0' + number % 10; // Rl[P(i)] возвращяем удобное для нас значения 
		if(i && !number) Rl[P(i)] = ' ';
		number /= 10;
	}
}
